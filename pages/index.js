...
export async function getStaticProps() {
  ...
  const posts = files.map((__filename) => {
    ...
    //returns the content in slug format
    return {
      slug,
      frontmatter,
    };
          ...
          <div className="">
            {posts.map(({ slug, frontmatter }) => (
              <div key={slug} className="py-4 px-4 md:px-5 md:py-5 hover:text-blue-700 m-2 mt-4 transition ease-in-out hover:scale-110 duration-300 flex shadow-lg border border-black bg-[#fffef9] rounded">
                <Link href={`/blog/${slug}`}>
                  <a>
                    <h1 className="justify-start flex items-center md:text-[20px] font-bold md:font-normal font-sora">{frontmatter.title}</h1>
                    <h2 className="text-[15px] md:text-[17px]">{frontmatter.metaDesc}</h2>
